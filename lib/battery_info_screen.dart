...
class BatteryInfoScreen extends StatefulWidget { // Определяем StatefulWidget для экрана информации о батарее.
  @override
  _BatteryInfoScreenState createState() => _BatteryInfoScreenState(); // Создаем состояние для BatteryInfoScreen.
}

class _BatteryInfoScreenState extends State<BatteryInfoScreen> { // Определяем состояние для экрана информации о батарее.
  final Battery _battery = Battery(); // Создаем экземпляр Battery для получения информации о батарее.
  ...
  void initState() { // Метод, вызываемый при инициализации состояния.
    ...
    _battery.onBatteryStateChanged.listen((BatteryState state) { // Подписываемся на изменения состояния батареи.
  ...
  Future<void> _getBatteryInfo() async { // Асинхронный метод для получения информации о батарее.
    try {
      final batteryLevel = await _battery.batteryLevel; // Получаем уровень заряда батареи.
      final batteryState = await _battery.batteryState; // Получаем состояние батареи.
      final AndroidBatteryInfo? androidBatteryInfo = await BatteryInfoPlugin().androidBatteryInfo; // Получаем информацию о батарее Android.
      ...
      if (mounted) { // Проверяем, что виджет все еще существует.
        setState(() { // Обновляем состояние.
          ...
          _batteryTemperature = androidBatteryInfo?.temperature?.toDouble() ?? 0.0; // Устанавливаем температуру батареи.
          _batteryVoltage = (androidBatteryInfo?.voltage ?? 0) / 1000; // Преобразуем мВ в В.
        });
      }
    } catch (e) {
      print('Ошибка получения информации об аккумуляторе: $e'); // Выводим ошибку в консоль.
    }
  }
  ...
  void _startUpdatingBatteryInfo() {
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) async { // Запускаем таймер, который срабатывает каждую секунду.
      await _getBatteryInfo(); // Обновляем информацию о батарее.

      if (timer.tick >= 10) { // После 10 секунд останавливаем таймер.
        timer.cancel();
        setState(() {
  ...
  String _getBatteryStateDescription(BatteryState state) { // Метод для получения описания состояния батареи.
    switch (state) {
  ...
  Color _getBatteryLevelColor(int level) { // Метод для получения цвета в зависимости от уровня заряда.
    if (level > 75) {
  ...
  @override
  void dispose() { // Метод, вызываемый при уничтожении виджета.
  ...
  Widget build(BuildContext context) { // Метод, который строит виджет.
    return Scaffold( // Создаем Scaffold для основной структуры страницы.
      ...
      body: SingleChildScrollView( // Позволяем прокручивать содержимое.
        ...
        child: Column( // Столбец для размещения информации о батарее.
          crossAxisAlignment: CrossAxisAlignment.start, // Выравнивание по левому краю.
          children: [
            // Индикатор уровня заряда батареи с цветовой индикацией.
            Center( // Центрируем индикатор.
              child: Stack( // Стек для наложения индикатора и текста.
                ...
                children: [
                  SizedBox(
                    ...
                    child: CircularProgressIndicator( // Индикатор загрузки.
                      ...
                      valueColor: AlwaysStoppedAnimation<Color>( // Цвет индикатора в зависимости от уровня.
                        _getBatteryLevelColor(_batteryLevel),
                      ),
                      backgroundColor: Colors.grey[200], // Цвет фона индикатора.
                    ),
                  ),
                  Text( // Отображаем уровень заряда в центре индикатора.
                    '$_batteryLevel%',
                    style: TextStyle(
                      fontSize: 24, // Размер шрифта.
                      fontWeight: FontWeight.bold, // Жирный шрифт.
                      color: _getBatteryLevelColor(_batteryLevel), // Цвет текста в зависимости от уровня.
                    ),
                  ),
                ],
              ),
            ...
            _buildInfoCard( // Создаем карточку с информацией о состоянии батареи.
              title: 'Состояние',
              value: _getBatteryStateDescription(_batteryState), // Описание состояния батареи.
            ...
            _buildInfoCard( // Создаем карточку с информацией о температуре.
              title: 'Температура',
              value: '$_batteryTemperature°C', // Температура батареи.
              icon: Icons.thermostat, // Иконка для температуры.
              backgroundColor: Color.fromARGB(255, 44, 110, 209), // Цвет фона карточки.
            ),
            SizedBox(height: 20), // Отступ после карточки.

            // Карточка с информацией о напряжении.
            _buildInfoCard( // Создаем карточку с информацией о напряжении.
              title: 'Напряжение',
            ...
            Align(
              alignment: Alignment.center,
              child: ElevatedButton.icon( // Кнопка с иконкой.
                onPressed: () {
                ...
                label: Text( // Текст на кнопке.
                  ...
                  style: TextStyle(fontSize: 16, color: Colors.white), // Стиль текста на кнопке.
                ),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color.fromARGB(255, 44, 110, 209), // Цвет фона кнопки.
                  padding: EdgeInsets.symmetric(horizontal: 32, vertical: 16), // Отступы внутри кнопки.
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(20), // Скругленные углы кнопки.
                  ),
                ),
              ),
            ),
  ...
  Widget _buildInfoCard({ // Метод для создания карточек с информацией.
    ...
    return Container( // Контейнер для карточки.
