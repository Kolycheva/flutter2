...
class _TestingScreenState extends State<TestingScreen> {
  ...
  Future<void> _initNetworkInfo() async {
    ...
    if (permissionStatus.isGranted) {
      try {
        ...
        developer.log('Не удалось получить информацию о сети', error: e);
      }
    } else {
      setState(() {
    ...
    try {
      _speedtest.getDataspeedtest(
        ...
        onError: (errorMessage) {
          developer.log('Ошибка при тестировании скорости: $errorMessage');
        },
        onDone: () => developer.log('Тест скорости завершён'),
      );
    } catch (e) {
      developer.log('Ошибка при тестировании: $e');
    }
  ...
  void _stopSpeedTest() {
    if (_isTesting) {
      ...
      _timer?.cancel();
      developer.log('Тест завершён через 40 секунд.');
    }
  }
  ...
  Future<void> _generateAndSavePDF() async {
    ...
    if (directory != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('PDF файл успешно сохранен в папке DCIM: ${file.path}'),
      ));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
